...
select C.ClassID, C.CourseID, C.startTime, C.endTime, C.location, I.fname as 'Instructor fname', I.sname as 'Instructor sname' from Instructor I, Teaches T, Class C where I.email = T.iEmail and C.ClassID = T.ClassID and getdate() > startTime and getdate() < endTime
select getdate()
select I.fname as 'Instructor fname', I.sname as 'Instructor sname', I.email
                        from Instructor I, Course C, Qualified Q 
                        where C.CourseID = Q.CourseID and Q.iEmail = I.email and C.CourseID = 'FA109'

select count(ClassID) as 'Number of Classes' from Class

select CourseID, count(distinct cEmail) from Attends A, Class C where A.ClassID = C.ClassID group by CourseID

select CourseID, avg(mark) from Attends A, Class C where A.ClassID = C.ClassID group by CourseID

select location, count(*) from Class group by location

select courseID, DATENAME(WEEKDAY, startTime), max(count)  from Class, (select count(*) as 'count' from Class group by courseID, DATENAME(WEEKDAY, startTime))t group by courseID, DATENAME(WEEKDAY, startTime)

select I.email, count(*) from Instructor I, Class C, Teaches T where I.email = T.iEmail and C.ClassID = T.ClassID group by I.email

select @@IDENTITY
select * from Class C, Teaches T, Instructor I where C.ClassID = T.ClassID and I.email = T.iEmail  
select * from Qualified