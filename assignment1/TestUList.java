// TestUList
// Test if all operations on a UnOrdStrList function correctly with a given test file.
// usage:  java TestUList <filename>
//
// 25 March 2023, Tony C Smith
//
// Reads input file one word/token at a time and if the word is not in the list then it is eadded, otherwise
// the word is removed.  After each token is processed, the list is printed out to the screen, along with
// the length of the list.  On completion, the list should be empty, and this is tested.
//
...
class TestUList{
    ...
    private static UnOrdStrList msl = new UnOrdStrList();
    
    public static void main(String [] args){
	if(args.length!=1){System.err.println("Usage:  java TestUList <filename>");return;}
